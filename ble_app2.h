...
public:
    ...
    /**
     * Set callback for a succesful Gatt Server notification/indication sent event.
     *
     * @param[in] cb The callback object that will be called when we connect to a peer
     */
    void on_serversentevent(mbed::Callback<void(const GattDataSentCallbackParams &params)> cb)
    {
        _post_serversentevents_cb = cb;
    }
...
protected:
    ...
    */
    ...
    /**
    * Handler called after an attribute has been written to.
    *
    ...
    */
    ...
    {
        if (_post_serverreadevents_cb) {
            _post_serversentevents_cb(params);
        }
    }
...
protected:
    ...
    mbed::Callback<void(const GattDataSentCallbackParams &params)> _post_serversentevents_cb;
